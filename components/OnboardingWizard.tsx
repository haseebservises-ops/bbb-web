...
type Scores = { exec: number; yo_yo_ozempic: number; longevity_seeker: number };
const baseScores: Scores = { exec: 0, yo_yo_ozempic: 0, longevity_seeker: 0 };

// Animations (let Motion infer types)
const variants = {
...
// 10-Q quiz bank
const questions = [
  { id: "q1",  text: "When stress hits, what's your usual response?", options: [
  ...
  { id: "q2",  text: "Pick the line that sounds most like you:", options: [
  ...
  { id: "q3",  text: "Your biggest blocker right now is…", options: [
  ...
  { id: "q4",  text: "How do you usually choose meals?", options: [
  ...
  { id: "q5",  text: "What would you like more of next month?", options: [
  ...
  { id: "q6",  text: "Pick the most helpful coach statement:", options: [
  ...
  { id: "q7",  text: "A win this week would be…", options: [
  ...
  { id: "q8",  text: "When travel or social events pop up…", options: [
  ...
  { id: "q9",  text: "How do you like to be coached?", options: [
  ...
  { id: "q10", text: "If BBB gave you one superpower, you'd pick…", options: [
...
export default function OnboardingWizard({ autoOpen = false, onClose, onComplete }: WizardProps) {
  ...
  const TOTAL = 4 + questions.length + 2; // welcome, goal, motivation, routine, 10Q, result, launch/grat
  ...
  const progress = useMemo(() => {
    ...
    return { value: clamped, total: TOTAL, pct: Math.round((clamped / (TOTAL - 1)) * 100) };
  }, [step]);

  const next = () => setStep((s) => s + 1);
  const back = () => setStep((s) => Math.max(0, s - 1));
  const setAnswer = (qi: number, oi: number) => { const copy = [...answers]; copy[qi] = oi; setAnswers(copy); next(); };

  function computeScores(): { scores: Scores; dominant: ArchetypeSlug } {
    const s: Scores = { ...baseScores };
    answers.forEach((oi, idx) => {
      const opt: QOption | undefined = oi >= 0 ? questions[idx].options[oi] : undefined;
      if (!opt) return;
  ...
  // close after 3s when on gratitude step (hooks cannot be inside conditionals)
  useEffect(() => {
    if (step !== FINAL_GRAT_STEP) return;
    const t = setTimeout(() => { setOpen(false); onClose?.(); }, 3000);
    return () => clearTimeout(t);
  }, [step, FINAL_GRAT_STEP, onClose]);
  ...
  const screen = (() => {
    ...
    if (step === 0) return (
      <motion.div key="welcome" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-4">
        ...
        <div className="flex gap-2 mt-2">
          ...
          <button onClick={() => { localStorage.setItem(LS_KEYS.dismissed, "1"); setOpen(false); onClose?.(); }} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-semibold">Skip for now</button>
        </div>
      </motion.div>
    );
      ...
      <motion.div key="goals" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-4">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {[
            ...
            <button key={g.slug} onClick={() => setGoal(g.slug as ArchetypeSlug)} className={cn("text-left rounded-2xl p-4 border transition", goal===g.slug ? "ring-4 ring-violet-200 border-violet-400" : "hover:shadow-md")}>
        ...
        <div className="flex justify-between pt-1">
          ...
          <button onClick={() => goal && next()} disabled={!goal} className="px-4 py-2 rounded-xl bg-violet-600 text-white font-bold disabled:opacity-40">Next</button>
        </div>
      </motion.div>
    );
    ...
    if (step === 2) return (
      <motion.div key="motivation" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-4">
        <div className="text-xl md:text-2xl font-extrabold">What would better {goal === "exec" ? "energy" : goal === "yo_yo_ozempic" ? "control" : "clarity"} let you do?</div>
        <textarea value={motivation} onChange={(e)=>setMotivation(e.target.value)} placeholder="Short sentence (optional)" className="w-full rounded-2xl border p-3 min-h-[100px]" />
        <div className="flex justify-between pt-1">
          <button onClick={back} className="text-slate-500">Back</button>
          <button onClick={next} className="px-4 py-2 rounded-xl bg-violet-600 text-white font-bold">Next</button>
        </div>
      </motion.div>
    );
    ...
    if (step === 3) return (
      <motion.div key="routine" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-6">
        ...
        <div className="grid gap-2">
          <label className="text-sm font-semibold text-slate-600">Meals per day: <span className="font-bold text-slate-800">{mealsPerDay}</span></label>
        ...
        <div className="grid gap-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            {["Low","Light","Moderate","High"].map((t,i)=> (
              <button key={t} onClick={()=>setActivity(i+1)} className={cn("rounded-xl p-3 border text-sm", activity===i+1 && "ring-4 ring-violet-200 border-violet-400 font-bold")}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-between pt-1">
          <button onClick={back} className="text-slate-500">Back</button>
          <button onClick={next} className="px-4 py-2 rounded-xl bg-violet-600 text-white font-bold">Start 60-second quiz</button>
        </div>
      </motion.div>
    );
    ...
    if (qIndex >= 0 && qIndex < questions.length) {
      ...
      return (
        <motion.div key={q.id} variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-5">
          <div className="text-sm text-slate-500">Question {qIndex+1} / {questions.length}</div>
          <div className="text-xl md:text-2xl font-extrabold leading-snug">{q.text}</div>
          <div className="grid gap-3">
            {q.options.map((opt, i) => (
              <button key={i} onClick={()=>setAnswer(qIndex, i)} className="text-left rounded-2xl p-4 border hover:shadow-md transition">
                {opt.label}
              </button>
            ))}
          </div>
    ...
    if (step === FINAL_RESULT_STEP) return (
      <motion.div key="result" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-5">
        ...
        <div className="grid gap-2 text-sm text-slate-600">
          <div>Focus preview: <span className="font-semibold">{goal || "—"}</span></div>
          <div>Scores → exec: {scores.exec} · yo-yo: {scores.yo_yo_ozempic} · longevity: {scores.longevity_seeker}</div>
        </div>
        <div className="flex flex-wrap gap-2">
          <button disabled={submitting} onClick={finish} className="px-4 py-2 rounded-xl bg-violet-600 text-white font-bold disabled:opacity-40">
            {submitting ? "Saving…" : "Save & continue"}
          </button>
          <button onClick={()=> setOpen(false)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-semibold">Close</button>
        </div>
      </motion.div>
    );

    // 15: Launch (“Open login / sign up”)
    if (step === FINAL_LAUNCH_STEP) return (
      <motion.div key="launch" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-4">
        ...
        <p className="text-slate-600">We saved your profile and added your onboarding bonus. Open the login window to start using Better Bite Buddy™.</p>
        <button
          onClick={() => {
  ...
  return (
    <>
      {!open && (
        <button onClick={()=>setOpen(true)} className="fixed bottom-5 right-5 z-40 bg-white/90 backdrop-blur px-4 py-2 rounded-xl border shadow">
        ...
        {open && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div className="w-[min(720px,95vw)] max-h-[88vh] overflow-auto bg-white rounded-3xl shadow-2xl p-6 md:p-7 grid gap-6 relative">
              <button onClick={()=>{ localStorage.setItem(LS_KEYS.dismissed,"1"); setOpen(false); onClose?.(); }}
                className="absolute right-3 top-3 w-9 h-9 grid place-items-center rounded-full hover:bg-slate-100 text-xl">×</button>
