...
/* Types that depend on questions */
type QOption = typeof questions[number]["options"][number];

/* --------------------------------- Steps --------------------------------- */
const STEP = {
  WELCOME: 0,
  PROMISE: 0.5 as number,
  GOAL: 1,
  MOTIVATION: 2,
  ROUTINE: 3,
  STATS: 3.5 as number,
...
const FIRST_QUIZ_STEP   = STEP.FIRST_Q;
const FINAL_RESULT_STEP = FIRST_QUIZ_STEP + questions.length;
const FINAL_LAUNCH_STEP = FINAL_RESULT_STEP + 1;
const FINAL_GRAT_STEP   = FINAL_RESULT_STEP + 2;
const TOTAL_SCREENS     = FINAL_GRAT_STEP + 1;
...
export default function OnboardingWizard({ autoOpen = false, onClose, onComplete }: WizardProps) {
  ...
  useEffect(() => {
    ...
    const progress = useMemo(() => {
      const clamped = Math.min(step, TOTAL_SCREENS - 1);
      return {
        value: clamped,
        total: TOTAL_SCREENS,
        pct: Math.round((clamped / (TOTAL_SCREENS - 1)) * 100),
      };
    }, [step]);
...
if (step === (3.5 as any)) return (
  <motion.div key="stats" variants={variants} initial="enter" animate="center" exit="exit" className="grid gap-6">
    ...
    <div className="flex justify-between pt-1">
      ...
      <button onClick={() => setStep(FIRST_QUIZ_STEP)} className="px-4 py-2 rounded-xl bg-violet-600 text-white font-bold">
        Continue
      </button>
    </div>
  </motion.div>
    ...
    // 4..13: 10 Questions
    const qIndex = step - FIRST_QUIZ_STEP;
